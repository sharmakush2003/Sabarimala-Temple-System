...
function renderVoucherHTML(recNo, dateStr, devoteeName, amountVal, amountWords, modeStr, sevaPurpose, lang = currentVoucherLang, type = "INWARD") {
    ...
    if (devoteeName && devoteeName.length > 0) {
        nameFontSize = Math.max(0.68, Math.min(1.05, 21.5 / devoteeName.length)) + "rem";
    }
    
    let wordsFontSize = "0.92rem";
    if (part1 && part1.length > 0) {
        wordsFontSize = Math.max(0.72, Math.min(0.92, 26 / part1.length)) + "rem";
    }
    
    if (type === "OUTWARD") {
        if (lang === 'kannada') {
            return `
            <div style="font-family: 'Noto Sans Kannada', 'Inter', sans-serif; max-width: 520px; width: 100%; box-sizing: border-box; margin: 0 auto;">
            ...
            return `
            <div style="font-family: 'Inter', sans-serif; max-width: 520px; width: 100%; box-sizing: border-box; margin: 0 auto;">
...
function renderModalVoucherContent(rec) {
    ...
    container.innerHTML = `
        <div class="dual-print-container" style="max-width: 520px; margin: 0 auto; display: flex; flex-direction: column; gap: 15px; box-sizing: border-box; background: white;">
